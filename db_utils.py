...
def get_last_sync(session: Session, sync_type: str) -> Optional[SyncLog]:
    ...
    return (
        ...
        .filter(SyncLog.sync_type == sync_type, SyncLog.success == 1)
...
def get_top_artists(
    ...
    if playlist_pattern:
        ...
        query = query.join(
            Playlist, Playlist.id == playlisttrack_association.c.playlist_id
        )
    ...
    return (
        query.group_by(Artist.id).order_by(text("track_count DESC")).limit(limit).all()
    )  # type:ignore
